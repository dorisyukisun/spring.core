...
//切面程式
@Component //可以被spring 管理的物件
@Aspect //切面
@Order (1) //數字越小越先執行(若不設定預設就是int的最大值)
		   //使用時機:有多個切面程式在同一個程式切面關注點要執行時
public class MyLoggerAspect  {
	...
	/*
	//後置通知 Advice:執行在連接點呼叫完之後, 在SpringAOP 機制上會配置在Finally 區段中
	@After(value = "pt()")
	public void afterAdvice(JoinPoint joinPoint) {
		String methodName = joinPoint.getSignature().getName();//取的連接點的方法簽章名稱
		System.out.printf("後置通知 -方法名稱: %s\n" , methodName);
		
	}
	//返回通知 Advice :可以設定 returning 來接收方法回傳值
	//若目標方法發生例外則不會執行返回通知
	@AfterReturning(value = "pt()",returning = "result")//設定將方法的回傳值放到result 變數中
	public void afterReturningAdvice(JoinPoint joinPoint, Object result) {
		String methodName = joinPoint.getSignature().getName();//取得連接點的方法簽章名稱
		System.out.printf("返回通知 -方法名稱: %s回傳值: %s\n",methodName,result);
		
	}
	//異常通知 Advice:可以設定throwing 來接收異常物件
